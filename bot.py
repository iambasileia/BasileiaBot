...
helptext = (
    ...
    ';getex - Which users are excepted from the filter.\n'
    ';setex - Add a user excepted from the filter.\n'
    ';rmex - Remove a user excepted from the filter.\n'
...
conn = psycopg2.connect(host = DATABASE_URL, user = DataSource(os.environ.get('DBUSER')), password = DataSource(os.environ.get('DBPASSWORD')), dbname = DataSource(os.environ.get('DBNAME')))
...
async def sendNotes(userID, channel):
    ...
    if int(userID) in exceptionlist:
